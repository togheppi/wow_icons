...
# Plot losses
def plot_loss(avg_losses, num_epochs, upsample, save=False, save_dir='results/', show=False):
    fig, ax = plt.subplots()
    ...
    for i in range(len(avg_losses)):
        if i % 2 == 0:
            plt.plot(avg_losses[i], label='D_loss_' + upsample)
        else:
            plt.plot(avg_losses[i], label='G_loss_' + upsample)
    plt.legend()
